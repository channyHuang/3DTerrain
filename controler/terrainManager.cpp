...
void TerrainManager::_process() {
    // send update request
    {
        ...
        for (auto itr = _blocks_pending_update.begin(); itr != _blocks_pending_update.end(); itr++) {
            ...
            TerrainStruct::InputBlock iblock;
        ...
        std::cout << __FILE__ << " " << __FUNCTION__ << " input " << input.blocks.size() << std::endl;
    ...
    // receive updated mesh
    {
        ...
        for (int i = 0; i < output.blocks.size(); ++i) {
            const TerrainStruct::OutputBlock &data = output.blocks[i];
            for (int j = 0; j < data.smooth_surfaces.surfaces.size(); ++j) {

                TerrainStruct::Arrays surface = data.smooth_surfaces.surfaces[j];
                if (surface.empty()) {
                    continue;
                }
